...
%       idx = [144,1:143]
%
% 2. 附件1、附件2、预测结果全部保持原始144列：
%       0:10, 0:20, ..., 23:50, 0:00+1
%
% 3. price = priceRaw
%
% 4. actualLoadRun = loadRaw(predDays,:)
%    actualPVRun   = pvRaw(predDays,:)
%
% 5. predLoadRun = predLoad
%    predPVRun   = predPV
%
% 6. 使用全新的TIMEFIX断点文件，
%    不读取以前错误时间轴下的调度结果
...
fprintf('====================================================\n');
fprintf(' C题问题2：16组合 TIMEFIX 热力图\n');
fprintf(' Q0.8 + 因果回放 + 允许同时充放电\n');
fprintf(' 时间轴保持附件原始144列顺序\n');
fprintf('====================================================\n\n');
...
resultMatFile = ...
    fullfile( ...
    baseDir, ...
    'Q2_16combo_TIMEFIX_FINAL_results.mat');


resultExcelFile = ...
    fullfile( ...
    baseDir, ...
    'Q2_16combo_TIMEFIX_FINAL_results.xlsx');
...
dispatchCheckpoint = ...
    fullfile( ...
    baseDir, ...
    'Q2_16combo_TIMEFIX_dispatch_checkpoint.mat');
...
figCostFile = ...
    fullfile( ...
    baseDir, ...
    'Q2_16combo_TIMEFIX_总成本热力图.png');


figEmergencyFile = ...
    fullfile( ...
    baseDir, ...
    'Q2_16combo_TIMEFIX_紧急购电热力图.png');


figSpillFile = ...
    fullfile( ...
    baseDir, ...
    'Q2_16combo_TIMEFIX_富余电量热力图.png');


figQ80File = ...
    fullfile( ...
    baseDir, ...
    'Q2_16combo_TIMEFIX_Q80热力图.png');
...
if exist('arima','file')~=2
    ...
    error( ...
        '缺少arima，需要Econometrics Toolbox。');
...
if exist('linprog','file')~=2
    ...
    error( ...
        '缺少linprog，需要Optimization Toolbox。');
...
if ~useExistingPrediction ...
        && isfile(oldPredictionFile)

    P = ...
        load(oldPredictionFile);
...
if ~useExistingPrediction
    ...
    if any(isnan(predLoad),'all') ...
            || any(isnan(predPV),'all')
...
fprintf('不进行 [144,1:143] 循环移位。\n');

fprintf('电价、真实负载、真实光伏、预测全部保持原顺序。\n');
...
if isfile(dispatchCheckpoint)
    ...
    if checkpointOK ...
        ...
        fprintf( ...
            '恢复TIMEFIX调度断点：组合%d/16\n', ...
            lastCombo);

    else

        fprintf( ...
            'TIMEFIX断点不完整，忽略并重新计算。\n');
